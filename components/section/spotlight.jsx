      ...
      <section className="bg-blackless pb-20 pt-12">
        <Wrapp>
          <div className="flex justify-between mb-2">
            ...
            <button className="border rounded-3xl px-4 lg:block hidden hover:bg-blue-700 py-1">
              See all Aalto University news
            </button>
          </div>
          <p className="mb-4">
            Finds more current news and interesting events on the{" "}
            <span className="inline-block underline">
              <a href="#">News and events page</a>
            </span>
            .
          </p>
            ...
            {Spotlight.map((item, idx) => (
              <a href="#">
                <div key={idx} className="border  pb-4 lg:mb-0 mb-4">
                  ...
                  <h2 className="text-xl pl-4 mt-2 font-semibold">
                    {item.title}
                  </h2>
                  <p className="pl-4 my-4">{item.describe}</p>
                  <p className="pl-4 mt-2">
                    {item.time + " "}
                    <span className="inline-block border-l pl-1">
                      {item.new}
                    </span>
                  </p>
                </div>
          ...
          <button className="border rounded-3xl px-4 block lg:hidden py-1">
            See all Aalto University news
          </button>
        </Wrapp>
      </section>
